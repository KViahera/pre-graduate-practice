...
import org.example.platform.security.JwtAuthenticationFilter;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable)
                .authorizeHttpRequests(auth -> auth
                        // 1. Публичные эндпоинты (Логин/Регистрация)
                        .requestMatchers("/api/auth/**").permitAll()

                        // 2. РАЗРЕШАЕМ GET-запросы к задачам для всех
                        .requestMatchers(org.springframework.http.HttpMethod.GET, "/api/problems/**").permitAll()

                        // 3. Всё остальное — по токену
